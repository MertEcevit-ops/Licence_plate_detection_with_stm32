...
int main(void)
{
  ...
  if (AppPlatform_Init() != APP_STATUS_OK)
  {
    Error_Handler();
  }
  ...
  if (AppCamera_InitDcmipp() != APP_STATUS_OK)
  {
    Error_Handler();
  }

  if (AppCamera_InitSensor() != APP_STATUS_OK)
  {
    Error_Handler();
  }
  ...
  if (AppTasks_Create() != APP_STATUS_OK)
  {
    Error_Handler();
  }

  AppTasks_StartScheduler();
  Error_Handler();
  /* USER CODE END 2 */
