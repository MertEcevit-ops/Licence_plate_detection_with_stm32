...
\texttt{alpr\_inference.py}. It loads a YOLO \texttt{.pt} detector, applies a
confidence threshold, crops detected plate regions, uses EasyOCR for text
recognition, and normalizes the output with a Turkish licence plate regular
expression. This host pipeline can process saved snapshots or other input
images while the embedded implementation continues to provide the camera
capture and secure transfer path.

\begin{lstlisting}[caption={Typical host commands for receiving a snapshot and running ALPR inference.}]
python Tools/receive_snapshot.py /dev/ttyACM0 -b 115200 -o snapshot.png
python alpr_inference.py --input snapshot.png --pt yolov8_plaka.pt --conf 0.25
\end{lstlisting}
...
\begin{itemize}[noitemsep]
  ...
  \item Update the standalone protocol documentation so it matches the current AES-128-CTR frame format.
\end{itemize}
